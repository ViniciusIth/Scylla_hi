import lightbulb
import hikari
from scylla.functions import member_functions as mf
...
@lightbulb.option(
	name="member",
	description="O nome do membro que terá suas informações extraídas",
	type=hikari.Member,
)
...
async def cmdMember(ctx: lightbulb.Context, member: hikari.Member = None) -> None:
	...
	message.add_field(
		name="Atividade", value=f'```{presence["activity"]}```', inline=False
	)
	message.add_field(
...
@lightbulb.command("permissions", "Pega as permissões de determinado usuário.")
@lightbulb.implements(lightbulb.PrefixSubCommand, lightbulb.SlashSubCommand)
async def memberPermission(ctx: lightbulb.Context, user: hikari.User = None) -> None:
	member: hikari.Member = ctx.app.cache.get_member(
		ctx.get_guild(), ctx.options.member or member
	)
...
async def memberAvatar(ctx: lightbulb.Context, user: hikari.User = None) -> None:
	...
	message = hikari.Embed(
		...
		description=f"Aqui está a imagem de perfil de {user.username}:",
	)
	message.set_image(user.avatar_url)
